...
export function useGetMyStats() { useVersion(); const all = sessions(); const totalPoints = all.reduce((s, x) => s + Number(x.pointsEarned || 0), 0); return { data: { totalPoints, spinsPlayed: all.filter(s => s.type === "spin").length, tongueTwisterSubmissions: all.filter(s => s.type === "tongue-twister").length, quizzesCompleted: all.filter(s => s.type === "quiz").length, rank: 1, recentActivity: all.slice(-5).reverse().map(s => ({ type: s.type === "memory" ? "quiz" : s.type, points: s.pointsEarned, createdAt: s.createdAt })) }, isLoading: false }; }
...
export function useSubmitMemoryGameSession() { return mutation<{ data: { maxLevel: number; pointsEarned: number } }, Session>(({ data }) => saveSession({ type: "memory", ...data })); }
export function useGetMemoryGameLeaderboard(_opts?: { params?: { limit?: number } }) { useVersion(); const user = read<User>(keys.user, seedUser); const mem = sessions().filter(s => s.type === "memory"); return { data: [{ rank: 1, userId: user.id, username: user.username, displayName: user.displayName, avatarUrl: user.avatarUrl, bestLevel: Math.max(0, ...mem.map(s => Number(s.maxLevel || 0))), gamesPlayed: mem.length }], isLoading: false }; }
